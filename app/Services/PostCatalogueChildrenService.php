...
class PostCatalogueChildrenService implements PostCatalogueChildrenServiceInterface
{
    ...
    public function paginate($request){
        ...
        $postCatalogueChildrens=$this->postCatalogueChildrenRepository->pagination(
            ...
            $condition, 
            $perpage, 
            ['path'=> 'post/CatalogueChildren/index'], 
            [],
            [], 
            ['posts']
        );
    ...
    public function updateStatus($post=[]){
        ...
        try{
            ...
            $condition = [
                ['post_catalogue_panrent_id', '=', [$post['modelId']]]
            ];
            $this->catalogueChildrenRepository->updateByWhere($condition, $payload);
            DB::commit();
            return true;
        }catch(\Exception $ex){
            DB::rollBack();
    ...
    private function paginateSelect(){
        return [
            'post_catalogue_children_id','post_catalogue_children_name','post_catalogue_children_description','publish'
        ];
    }
