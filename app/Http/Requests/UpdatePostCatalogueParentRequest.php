...
class UpdatePostCatalogueParentRequest extends FormRequest
{
    ...
    {
        return [
            'post_catalogue_parent_name' => 'required | string | unique:post_catalogue_parent,post_catalogue_parent_name, '.$this->id.' | min: 2 | max: 255 | regex:/^[^<>&]*$/',
            'post_catalogue_parent_description' => 'string | regex:/^[^<>&]*$/ '
        ];
    }
